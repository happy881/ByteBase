<style scoped>
  ...
  .fileBoxRight:hover .keyimg {
    background-color: #EF190F;
    transition: all 1s;
  }
  .fileBoxRight:hover .fzkeyword {
    color: #EF190F;
...
<template>
  <div class="allFileApp" @click="quanjuFun" v-loading="loading" element-loading-text="解密下载中！" element-loading-spinner="el-icon-loading"
       element-loading-background="rgba(255, 255, 255, 0.8)">
    <div class="lable">
      <el-dialog
        :title="$t('privateKey')"
        :visible.sync="changebtn"
        width="400px"
        :center="true"
      >
        <div class="ltitke">
        ...
        <input
          ...
          class="Lbainput"
          type="text"
          v-model="submitkeystr"
        />
        <div class="Lablesubmit" @click="changesubmit">{{ $t('confirms') }}</div>
      </el-dialog>
    </div>
    ...
    >
      ...
      <div class="upFileType" :class="{ upFileTypeFile: drogBoolean }">
        ...
        <div class="tips">{{ $t('TipsContent') }}</div>
        ...
        >
          <div class="fileBoxItem_1">
            ...
            <div>
              ...
              <div class="fileBoxTime" v-if="item.loading && !item.loadkyes">
          ...
          >
            <div class="keyimg" v-if="item.succese" @click="fzkeyword(item.keyword)">
              <img src="../assets/byteicon/keyword1.png" alt="" style="width:12px">
            </div>

            <div class="fzkeyword" v-if="item.succese" @click="fzkeyword(item.keyword)">{{ $t('copyKey') }}</div>
    ...
    <div class="smdialog">
      <el-dialog
        ...
        :visible.sync="openSm"
        :center="true"
        :before-close="closeSmdialog"
      >
        <div class="smmidwarp" style="">
          <img
            class="smimg2"
            src="../assets/byteicon/encryption.png"
            alt=""
          />
          <div class="smrzfontwarp">
<!--            <div class="warptitme"></div>-->
            <div class="warptitme2">
              {{ $t('TipsContent') }}
            </div>
          </div>
        </div>
        <div class="Smidbottom">
          <div class="check">
            <el-checkbox fill="#ef190f" v-model="checked"></el-checkbox>
            <div class="nosee">{{ $t('noMore') }}</div>
          </div>
          <div class="btns">
            <div class="cnacel-btn" @click="openSm = false">{{ $t('cancel') }}</div>
            <div class="gosm" @click="rzclick">{{ $t('knowned') }}</div>
          </div>
        </div>
      </el-dialog>
    </div>
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange (val) {
      ...
      // this.getAllFiles(val, 50)
      this.getFileListFun()
    },
    // 判断提示框显示
    showOrNot () {
      let sign = sessionStorage.getItem('flag')
      if (sign) {
        this.openSm = false
      } else {
        this.openSm = true
    ...
    changesubmit () {
      console.log(this.submitkeystr)
      this.strlength = this.submitkeystr
      if ((this.submitkeystr = '' || this.submitkeystr.length !== 16)) {
        this.submitkeystr = ''
        this.$message.error({
          content: this.$t('hint37')
        })
      } else {
        this.submitkeystr = ''
        this.changebtn = false
        this.downTokenGetFun(this.Item)
      }
    },
    ...
    async getAllFile (prefix, type) {
      ...
      await this.$axios({
        ...
        .then((res) => {
          ...
          console.log('xianzai de :', this.arrayA)
    ...
    downTokenGetFun (item) {
      ...
      this.$axios({
        ...
        .then((res) => {
          ...
          this.downAESurl(ipfsUrl, item.name)

          // if (item.fileTypeName === 0) {
          //     console.log(0)
          //     let blob = new Blob2([response.data], {
    ...
    dataURLtoBlob (code) {
      let baseurl = code.substring(1, code.length - 1)
      console.log(baseurl)
      let parts = baseurl.split(';base64,')
      console.log(parts)
    ...
    downAESurl (url, name) {
      ...
      axios({
        ...
        let base64String = this.arrayBufferToBase64(buf)
        console.log(base64String)
        let encrypts = base64String
        console.log(self.strlength)
        let key = self.strlength
        var dess = AES.decrypt(encrypts, key)
        if (!dess) {
          console.log('cuole')
          that.loading = false
          this.$message.info({
        ...
        // console.log(dess)
        let URL = this.dataURLtoBlob(dess)
    ...
    dataURLtoFile (dataurl, filename, type) {
      ...
      return new File([u8arr], (filename + '.enc'), { type: type })
    },
    uploadFile: function (file) {
      ...
      if (flag) {
        ...
        reader.onload = function () {
          var keys = AES.generatekey(16) // 生成密钥
          item.keyword = keys
          ...
          let ki = [file.name, keys, nowTime]
          // let ki = [file.name, keys]
          that.keyslist.push(ki)
          that.keysfilename = that.bucketName + '_' + nowTime
          console.log(that.keyslist)
          var readdata = reader.result // base64形式的文件内容
          // console.log(readdata);
          var encrypts = AES.encrypt(JSON.stringify(reader.result), keys) // 文件AES加密
          var dess = JSON.parse(AES.decrypt(encrypts, keys)) // 解密
          // console.log(encrypts);
          // console.log(encrypts.length);
          // console.log(dess);
          var array = []
          var base64String = dess
          var bytes = base64String.split(',')[1]
          let name = file.name
          console.log(name, file.type)
          let filename = name.substring(0, name.lastIndexOf('.'))
          let fileaddres = name.substring(name.lastIndexOf('.'))
          encrypts = 'data:' + file.type + ';' + 'base64,' + encrypts // 把加密后的字符串加上base64的头部
          console.log('我打印一下', encrypts)
          var newfd = that.dataURLtoFile(encrypts, name, file.type) // 把生成后的加密base64生成from-data对象
          // console.log('打印newfd',newfd)
          let filess = new window.File([newfd], name, {
            type: file.type
          }) // 把from-data对象转换为file对象
          let token = localStorage.token
          that.source = that.$axios.CancelToken.source() // 这里初始化source对象
          let prefix = ''
          let prefixName = ''
          console.log(that.arrangementList)
          for (let i = 1; i < that.arrangementList.length; i++) {
            prefix = prefix + that.arrangementList[i]
          }
          ...
          let minioToekn = localStorage.minioToekn
          let moment =
            that.$moment().utc().format('YYYYMMDDTHHmmss') + 'Z'

          console.log(that.dtcomm)
          that.$axios({
            method: 'PUT',
            url: base.homeMinioUrl +
              '/api/upload/' +
              that.bucketName +
              '/' +
              objectName + '.enc',
            ...
            .then((res) => {
              ...
              that.showKey = true
              that.cebianDataFun()
              if (that.openMaskControl.upBoxBoolean) {
                that.getFileListFun(prefix)
              }
            })
            .catch((err) => {
              console.log('fff')
    ...
    cellUp (item, index) {
      ...
      if (this.source) {
        ...
        this.source.cancel(this.$t('cancel')) // "取消上传"这几个字，会在上面catch（）的error中输出的，可以console看一下。
      }
    },
    onDrag: function (e) {
      e.stopPropagation()
    ...
    bucktlistFun () {
      ...
      this.$axios({
        ...
        .then((res) => {
          if (res.data.result.buckets !== null && res.data.result.buckets.length > 0) {
    ...
    changeTagFun (command) {
      ...
      if (this.arrangementList.length > 1) {
        if (command.name === '全部') {
          ...
          console.log('neirong:', this.arrayA)
        }
      }
    }
  }
